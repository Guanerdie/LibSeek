...
仓库同时包含 `android-app/` 下的原生 Android 客户端，默认连接
`https://unin.tlovex.de/`，支持手机和平板响应式布局。可安装的本地分发包构建说明见
`android-app/README.md`；账号、会话和第三方服务凭据不会写入 APK。
...
提交满足条件的候选。电视剧自动化不依赖本地缺集列表，只接受可确认的全集包或完整季包，
明显单集资源不会自动提交；下载完成后的本地文件替换由用户处理。live 候选必须带有精确
匹配的 TMDB 或 IMDb 身份；qB POST 前会按最新策略复验，并以种子内容的实际体积检查单
资源和每日预算。缺少 TMDB ID 时只在 TMDB 返回唯一结果后继续；配置或身份风险不会自动
重试，临时网络错误使用指数退避。
手动启动自动搜索时，服务端会先返回持久化运行 ID，再在后台执行；失败任务的手动重试会
立即创建新的运行与任务记录，不会改写原运行历史。网页和 Android 客户端
按运行 ID 轮询进度，因此长任务不受反向代理请求超时影响，离开页面后也可恢复查看。
