...
客户端覆盖登录、缺失影视同步、TMDB 确认、PT 候选搜索、qBittorrent 下载状态、自动化策略、服务配置和连接测试。管理员可在 App 内配置 NextFind、TMDB、出站代理、AvistaZ/NexusPHP 与 qBittorrent；敏感字段只用于一次写入，不会由服务端回显，也不会写入 Android 本地状态或日志。
...
```text
D:\project\unin\output\android\UNIN-1.1.0.apk
```

使用 ADB 安装：

```powershell
adb install -r D:\project\unin\output\android\UNIN-1.1.0.apk
```
